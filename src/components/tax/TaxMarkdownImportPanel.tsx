...
export function TaxMarkdownImportPanel({ existingBanks, onClose, onImport }: Props) {
  ...
  const overview = useMemo(() => {
    ...
    return {
      ...
      normalCount: draft.questions.filter((question) => question.parseWarnings.length === 0).length,
  ...
  const visibleQuestions = useMemo(() => {
    ...
    if (warningFilter === '选项不足') {
      return draft.questions.filter((question) =>
        question.parseWarnings.includes('客观题选项数量不足')
        || question.parseWarnings.includes('未识别选项'))
    }
    if (warningFilter === '无解析') {
      return draft.questions.filter((question) => question.parseWarnings.includes('未识别解析'))
    }
  ...
  function updateQuestion(questionId: string, updater: (question: ParsedTaxQuestion) => ParsedTaxQuestion) {
    setDraft((current) => current ? {
      ...
      questions: current.questions.map((question) =>
        question.questionId === questionId ? revalidateParsedTaxQuestion(updater(question)) : question),
    } : current)
  }
  ...
  function selectWarningFree() {
    ...
    setSelectedIds(new Set(
      draft.questions
        .filter((question) => question.parseWarnings.length === 0)
  ...
  function confirmImport() {
    ...
    if (invalid.length) {
      setMessage(`仍有 ${invalid.length} 道题缺少答案、解析、章节或必要选项，请修正后再导入。`)
      return
    }
...
function QuestionEditor({
  ...
  return (
    <article className={`border bg-white shadow-sm ${question.parseWarnings.length ? 'border-amber-300' : selected ? 'border-emerald-300' : 'border-slate-200'}`}>
...
function Summary({ label, value, warning }: { label: string; value: number; warning?: boolean }) {
  return <div className={`rounded-md px-3 py-3 ${warning && value ? 'bg-amber-50 text-amber-800' : 'bg-slate-50 text-slate-700'}`}><p className="text-xs">{label}</p><p className="mt-1 text-lg font-semibold">{value}</p></div>
}

function Message({ text }: { text: string }) {
  return <div className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800"><AlertTriangle size={16} className="mt-0.5 shrink-0" />{text}</div>
}
