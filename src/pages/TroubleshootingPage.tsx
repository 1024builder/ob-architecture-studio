import { AlertTriangle, CheckCircle2, Database, Search, ShieldCheck, X } from 'lucide-react'
...
export function TroubleshootingPage() {
  ...
  const databaseTypes = useMemo(() => unique(troubleshootingCases.map((item) => item.databaseType)), [])
  const faultTypes = useMemo(() => unique(troubleshootingCases.map((item) => item.faultType)), [])
  const filteredCases = useMemo(() => {
    const query = searchQuery.trim().toLocaleLowerCase()
    return troubleshootingCases.filter((item) => {
  ...
  }, [databaseType, faultType, searchQuery, severity])
  ...
  const highSeverityCount = troubleshootingCases.filter((item) => item.severity === '高').length
  const resolvedCount = troubleshootingCases.filter((item) => item.status === '已解决').length
  ...
  return (
    <div className="space-y-5 p-3 sm:p-4 lg:p-5">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric icon={Database} label="总案例数" value={troubleshootingCases.length} tone="ocean" />
      ...
      <section className="grid items-start gap-5 xl:grid-cols-[minmax(18rem,0.72fr)_minmax(0,1.6fr)]">
        <div className="xl:sticky xl:top-24">
          <div className="mb-3 flex items-end justify-between">
            ...
            <span className="text-xs text-slate-400">{filteredCases.length} / {troubleshootingCases.length}</span>
          </div>
